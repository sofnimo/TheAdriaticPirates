 ...
 * next to an islet resolved to centimetres, so mesh density is shared out by land area.
 */
...
export class Archipelago {
  ...
  refreshCover(structural = false): void {
    ...
    // NO REBUILD ON A STRUCTURAL EDIT ANY MORE. This used to re-scatter the canopy, because
    // hull placement was baked from the masks and a mask edit moved every crown. With the
    // canopy gone, nothing downstream of the cover bake is geometry — the masks are read as
    // textures by the land shaders and a uniform sync is the whole update.
  }

  setTierVisibility(overlay: boolean): void {
    for (const island of this.islands) island.setTierVisibility(overlay);
  }
