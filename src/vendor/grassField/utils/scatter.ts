...
export function samplePoint(
  ...
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    // ADRIATIC: non-null assertions throughout this function. `mid`/`t` are both
    // derived from the array's own length, so every read is in range; this repo
    // compiles with noUncheckedIndexedAccess and the alternative — a `?? 0` on each
    // of the nine reads below — would put a branch in the inner loop of a 50k-blade
    // scatter for a case that cannot happen.
