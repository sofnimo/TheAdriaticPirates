...
export function makeBladeMaterial(u: BladeUniforms): THREE.MeshLambertMaterial {
  ...
  // Upstream deliberately omitted a cache key here: every blade material injects
  // identical GLSL, so they must SHARE one compiled program, and a per-instance
  // key would compile a separate program per patch for no benefit — the per-field
  // values ride on uniforms, which are uploaded per material even on a shared one.
  //
  // ADRIATIC: that reasoning is right and the omission was not. Sharing between
  // BLADES is what was wanted; sharing with any OTHER material that happens to be
  // constructed with the same flags is not, and three cannot tell the difference
  // because its cache key knows nothing about onBeforeCompile. A blade is
  // DoubleSide with no map — so is a flower. A constant key keeps every blade on
  // one program and keeps the flowers off it.
